...
import {
  getUserWithSupabaseAccessToken,
  signInWithSupabase,
  signUpWithSupabase,
...
// Exchange a valid AfuChat Supabase session for the short-lived AfuCloud API
// session used by storage and other AfuCloud-owned resources.
auth.post("/session", async (c) => {
  const token = extractBearerToken(c.req.header("Authorization") ?? null);
  if (!token) return c.json({ error: "Supabase bearer token is required" }, 401);

  const authUser = await getUserWithSupabaseAccessToken(c.env, token);
  if (!authUser) return c.json({ error: "Invalid or expired Supabase session" }, 401);
