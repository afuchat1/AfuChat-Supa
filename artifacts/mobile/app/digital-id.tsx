...
function GradeInfo(grade: string): { label: string; colors: [string, string]; icon: string; textColor: string } {
  const map: Record<string, { label: string; colors: [string, string]; icon: string; textColor: string }> = {
    ...
    platinum: { label: "Platinum", colors: ["#00BCD4", "#0097A7"], icon: "star", textColor: "#0097A7" },
      ...
      <View style={[styles.cardContent, { padding: Math.max(12, 20 * s) }]}>
        <View style={styles.cardTopRow}>
          <View style={styles.brandRow}>
            <Image source={afuSymbol} style={[styles.brandSymbol, { width: 26 * s, height: 26 * s, borderRadius: 7 * s, tintColor: "#00BCD4" }]} resizeMode="contain" />
            <View>
        ...
        <View style={styles.cardMidRow}>
          ...
          {(profile?.country || profile?.region) ? (
            <View style={styles.locationRow}>
              <Ionicons name="location" size={Math.max(8, 10 * s)} color="#00BCD4" />
...
const styles = StyleSheet.create({
  ...
  printCardBtn: { borderRadius: 16, overflow: "hidden", borderWidth: 2, borderColor: "#00BCD4" },
  printCardInner: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 13, paddingHorizontal: 18 },
  printCardText: { color: "#00BCD4", fontSize: 15, fontFamily: "Inter_600SemiBold" },
