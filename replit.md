...
- **Navigation**: Expo Router with tab-based navigation and deep linking support. Bottom tabs (mobile + ClassicTabs/NativeTabs in `app/(tabs)/_layout.tsx`, `tabW = SW/5`): Chats, Discover, Communities, Apps, Me. Desktop sidebar (`components/desktop/DesktopSidebar.tsx`) mirrors these and additionally surfaces Shorts (`/discover?tab=shorts`). Post creation via FAB on discover page (hidden when the Shorts sub-tab is active). Communities (`app/(tabs)/communities.tsx`) shows the user's joined paid communities + a browse grid (1/2/3 cols by viewport) and links into `/paid-communities` for joining and creating.
...
- **Discover** (`app/(tabs)/discover.tsx`) — header has no border-bottom, "For You" / "Following" / "Shorts" are YouTube-style underline tabs (3px bottom-border + bold text on the active tab; the Shorts pill includes a small play-circle icon). Post cards sit on the page background with no surface chip and no top border on the footer action row. The Shorts tab swaps the post `FlatList` for `<ShortsFeed />` (a vertical 9:16 player feed: HTML5 `<video>` on web with `preload="auto"` + `playsInline`, expo-av on native, sourced via `useResolvedVideoSource` for R2 H.264/AV1 renditions — `targetHeight=480` on mobile for faster start — with tap-to-pause, mute toggle, like/comment/bookmark/share rail, and ±2 neighbour preload via a hidden muted/paused player). `ShortsFeed` accepts `layout: "fullscreen" | "card"` and `filter: "for_you" | "following"` props; bookmarks toggle rows in `post_bookmarks` (upsert on `post_id,user_id`).
  - **Mobile Shorts (TikTok-style fullscreen)**: when `feedTab === "shorts" && !isDesktop`, `discover.tsx` early-returns a black edge-to-edge container with `<ShortsFeed layout="fullscreen" filter={shortsFilter} bottomInset={insets.bottom} />`. A `useEffect` hides the parent ClassicTabs bar via `navigation.getParent()?.setOptions({ tabBarStyle: { display: 'none' } })` on enter and restores it on exit/unmount (NativeTabs on iOS cannot be hidden — accepted limitation). An absolute top mini-header overlays the player with: back chevron (returns to For You feed), centered "For You / Following" pills with white underline driving local `shortsFilter` state, and a search icon routing to `/(tabs)/search`. The right action rail (avatar + red follow `+`, heart/likes, comment/replies, bookmark, share) is overlaid on the video; bottom-left shows `@author`, caption, and `♪ original audio` attribution. Desktop Shorts keeps the default `layout="card"` inside `<DesktopFeedLayout>` (9:16 card with side rail) — unchanged.
