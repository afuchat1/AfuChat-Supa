...
- **Discover** (`app/(tabs)/discover.tsx`) — header has no border-bottom, "For You" / "Following" / "Shorts" are YouTube-style underline tabs (3px bottom-border + bold text on the active tab; the Shorts pill includes a small play-circle icon). Post cards sit on the page background with no surface chip and no top border on the footer action row. The Shorts tab swaps the post `FlatList` for `<ShortsFeed />` (a vertical 9:16 player feed: HTML5 `<video>` on web with `preload="auto"` + `playsInline`, expo-av on native, sourced via `useResolvedVideoSource` for R2 H.264/AV1 renditions, with tap-to-pause, mute toggle, and a like/comment/share rail).
  - On desktop (`useIsDesktop()` true), the body is wrapped in `<DesktopFeedLayout>` which renders the feed inside a centered 620px column with hairline left/right borders, plus a 340px right rail (`<RightRail />`) when the viewport is ≥1180px. The right rail surfaces "Who to follow" (suggested profiles the signed-in user does not yet follow) and "Trending" (top hashtags extracted from posts in the last 7 days) with footer links (Terms · Privacy · About · © year). The FAB and the bottom Communities tab stay outside this wrapper. On mobile the wrapper is a no-op.
