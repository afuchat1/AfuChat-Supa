...
- **Articles & Videos (post_type system)**: Posts table extended with `post_type` (`post`|`article`|`video`), `article_title`, and `video_url` columns (migration: `supabase/migrations/20240401_articles_videos.sql`). Article creation screen (`app/moments/create-article.tsx`) with title + body + audience picker + word count. Video creation screen (`app/moments/create-video.tsx`) with expo-image-picker, expo-av preview, 90-second max, upload to Supabase `videos` storage bucket. Article detail screen (`app/article/[id].tsx`) with full reading view, estimated read time, likes, comments. VideoFeed component (`components/VideoFeed.tsx`) for full-screen vertical snap-scroll autoplay with like/comment overlay. Discover FAB now opens a creation type picker (Post/Article/Video). Discover header has a "Videos" tab that shows the VideoFeed. Article cards in feed show badge, title, excerpt, and "Read Article" CTA. Opening an article navigates to `/article/[id]`.
