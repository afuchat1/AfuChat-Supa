...
- **Authentication**: Supabase Auth with email/password, Google/GitHub OAuth, OTP-based password reset, email confirmation. Supports multi-account switching. Phone sign-in uses Twilio Verify; while the project's Twilio account is on the trial plan it can only SMS pre-verified numbers, so `app/(auth)/phone.tsx` shows a friendly explanation and points testers at the Supabase test-OTP fallback `+256 705 085055` / code `777777` (configured via `sms_test_otp`, valid through 2027-12-31).
...
- **Articles & Videos (post_type system)**: Posts table supports `post`, `article`, `video` types. Article creation with title, body, audience, word count. Video creation with `expo-image-picker` on native and a hidden `<input type="file" accept="video/*">` plus an HTML5 `<video>` metadata probe (duration / dimensions) on web — both feed into `expo-av` preview and upload to Supabase Storage. The TikTok-style full-screen video player (`app/video/[id].tsx`) now renders on web too (the previous "Videos are only available in the app" guard was removed) since `expo-av`'s `<Video>` component is fully web-compatible.
