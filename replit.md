...
- **Threaded replies (video comments)**: `VideoReplyItem` component renders recursive threaded replies with depth-based indentation (max 3 levels, 24px per level) in the video CommentsSheet. Each reply has a "Reply" button to start a thread. `replyingTo` state shows a banner above the input with "@handle" and close button. Composer state (text + replyingTo) resets when sheet closes or postId changes, preventing cross-post stale reply targets. Top-level replies scroll to end; threaded replies don't force scroll.
- **Music marquee (video player)**: Bottom of video player shows a music row with musical note icon + audio name. Uses `audio_name` column from `posts` table; falls back to "Original audio · displayName". Positioned above the progress bar.
